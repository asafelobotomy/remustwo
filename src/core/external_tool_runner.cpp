...
namespace {
    ...
    QString hostProgram(const QString &name) {
        ...
        for (const QString &dir :
            { QStringLiteral("/usr/bin"), QStringLiteral("/usr/local/bin"), QStringLiteral("/bin") }) {
        ...
        for (const QString &root : flatpakAppRoots()) {
            ...
            for (const QString &appId : apps) {
                ...
                if (appId.contains(toolName, Qt::CaseInsensitive)
                    || (toolName.contains(QLatin1String("dolphin"))
                        && appId.contains(QLatin1String("dolphin"), Qt::CaseInsensitive))
                    || (toolName == QLatin1String("chdman")
                        && appId.contains(QLatin1String("mame"), Qt::CaseInsensitive))) {
                    return appId;
                }
                if (fallback.isEmpty())
                    fallback = appId;
